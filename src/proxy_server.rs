...
use crate::config::{Config, DomainOverride, Mode};
...
fn matches_sni_rewrite(host: &str, youtube_via_relay: bool) -> bool {
    ...
    SNI_REWRITE_SUFFIXES
        ...
        .filter(|s| {
            ...
            // trade-off; see `youtube_via_relay` in config.
...
impl ProxyServer {
    ...
    pub async fn run(
        ...
        // Local status API (best-effort).
        {
            ...
            let socks_listen = (self.host.clone(), self.socks5_port);
...
async fn dispatch_tunnel(
    ...
    match decision {
        ...
        RouteDecision::SniRewrite => {
            ...
            do_sni_rewrite_tunnel_from_tcp(sock, &host, port, mitm, rewrite_ctx).await
        }
        RouteDecision::MitmRelayTls => {
            ...
            tracing::info!(
                "dispatch {}:{} -> MITM + Apps Script relay (TLS)",
                host,
                port
            );
...
fn lan_auth_ok(sock: &TcpStream, headers: &[(String, String)], ctx: &RewriteCtx) -> bool {
    // Allowlist: exact IP match only (minimal baseline guardrail).
...
mod tests {
    ...
    fn youtube_via_relay_routes_youtube_through_relay_path() {
        // When youtube_via_relay=true, YouTube suffixes
