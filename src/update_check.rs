...
use crate::branding::{GITHUB_OWNER, GITHUB_REPO};

const REPO_OWNER: &str = GITHUB_OWNER;
const REPO_NAME: &str = GITHUB_REPO;
...
async fn https_raw_get(
    ...
    let req = format!(
        "GET {path} HTTP/1.1\r\n\
         Host: {host}\r\n\
         User-Agent: mhrv-f/{ver} (update-check)\r\n\
         Accept: {accept}\r\n\
         Connection: close\r\n\
         \r\n",
        path = path,
...
mod tests {
    ...
    fn pick_asset_prefers_app_zip_on_macos() {
        let assets = serde_json::json!([
            {"name": "mhrv-f-linux-amd64.tar.gz", "browser_download_url": "https://x/a", "size": 1},
            {"name": "mhrv-f-macos-arm64.tar.gz", "browser_download_url": "https://x/b", "size": 2},
            {"name": "mhrv-f-macos-arm64-app.zip", "browser_download_url": "https://x/c", "size": 3},
        ]);
        let arr = assets.as_array().unwrap();
        if cfg!(all(target_os = "macos", target_arch = "aarch64")) {
            let picked = pick_asset_for_platform(arr).expect("should pick");
            assert_eq!(picked.name, "mhrv-f-macos-arm64-app.zip");
        }
    }
