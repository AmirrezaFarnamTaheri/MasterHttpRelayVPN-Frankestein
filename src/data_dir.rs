...
const APP_NAME: &str = "mhrv-f";

/// Global override. On Android the app sets this to its private files dir
/// before any other mhrv-f code runs — avoids `directories` crate returning
...
/// - Linux:   `~/.config/mhrv-f` (or `$XDG_CONFIG_HOME/mhrv-f`)
...
/// Otherwise look in the user-data dir first, then `./config.json` in the
