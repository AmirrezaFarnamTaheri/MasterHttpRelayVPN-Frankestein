...
//   1. `__android_log_write` — lands in `adb logcat` under tag `mhrv_jni`.
...
pub extern "system" fn Java_com_therealaleph_mhrv_Native_startProxy(
    ...
    safe(
        ...
        AssertUnwindSafe(|| {
            ...
            let config: Config = match Config::from_json_str(&json) {
                Ok(c) => c,
                Err(e) => {
...
/// Address already in use" bug. 5s is enough for a cooperative server to
