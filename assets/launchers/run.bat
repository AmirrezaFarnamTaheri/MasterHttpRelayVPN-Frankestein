@echo off
REM mhrv-f launcher for Windows.
...
if not exist "mhrv-f.exe" (
    echo error: mhrv-f.exe not found next to this script.
    pause
    exit /b 1
)
...
REM `start "" "mhrv-f-ui.exe"` which returns immediately and swallows any
REM launch-time crash.
mhrv-f-ui.exe
