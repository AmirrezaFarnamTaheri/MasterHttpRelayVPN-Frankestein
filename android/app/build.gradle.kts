...
android {
    ...
    // libmhrv_jni.so is produced by `cargo ndk` in the repo root and dropped
...
tasks.register<Exec>("cargoBuildDebug") {
    ...
    description = "Cross-compile mhrv_jni for all ABIs (release — same as cargoBuildRelease)"
...
tasks.register<Exec>("cargoBuildRelease") {
    group = "build"
    description = "Cross-compile mhrv_jni for all ABIs (release)"
    workingDir = rustCrateDir
    commandLine(buildList<String> {
        add("cargo"); add("ndk")
        androidAbis.forEach { add("-t"); add(it) }
