 ...
 * libmhrv_jni) defines its C entry points under this exact package path:
 ...
 *
 * NOTE: the tun2proxy JNI symbols live in libtun2proxy.so (not libmhrv_jni.so).
 * tun2proxy is pulled in as a Rust dep of mhrv-f, but because nothing in
 * mhrv-f calls these symbols directly, Rust's rlib-level dead-code
 * elimination drops them from libmhrv_jni.so. The cdylib variant of tun2proxy
