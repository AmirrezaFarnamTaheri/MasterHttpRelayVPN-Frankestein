...
/// never be a real destination. Use an address near the end of the range
/// to avoid tun2proxy virtual-DNS fake IPs, which commonly start near
/// `198.18.0.1`.
pub const UDPGW_MAGIC_IP: [u8; 4] = [198, 19, 255, 254];
...
pub fn is_udpgw_dest(host: &str, port: u16) -> bool {
    port == UDPGW_MAGIC_PORT
        && host
            == format!(
                "{}.{}.{}.{}",
                UDPGW_MAGIC_IP[0], UDPGW_MAGIC_IP[1], UDPGW_MAGIC_IP[2], UDPGW_MAGIC_IP[3]
            )
}
...
mod tests {
    ...
    fn is_udpgw_dest_works() {
        assert!(is_udpgw_dest("198.19.255.254", 7300));
        assert!(!is_udpgw_dest("198.19.255.254", 80));
