...
fn health_details() -> HealthDetails {
    HealthDetails {
        status: "ok",
...
async fn handle_health_details() -> Json<HealthDetails> {
    Json(health_details())
}
...
///
/// 15 s favors push delivery and fewer empty round-trips while staying safely
/// below the client's `BATCH_TIMEOUT` (30 s) and Apps Script's UrlFetch
/// ceiling (~60 s).
const LONGPOLL_DEADLINE: Duration = Duration::from_secs(15);

/// Upper bound on bytes returned for one TCP session in one batch drain.
///
/// A fast tunnel-node can fill tens of MiB between Apps Script polls. Returning
/// that whole buffer in one JSON/base64 response can exceed Apps Script's body
/// ceiling, so large buffers are split across successive drains. This is a
/// per-session cap; a batch with several hot sessions can still return more.
...
///
/// This replaces the older fixed-delay retry pattern
...
async fn handle_batch(State(state): State<AppState>, body: Bytes) -> impl IntoResponse {
    ...
    //     the first notify. For active batches we adaptively settle for
    //     trailing neighbors so replies that arrive just after the first
    //     notify are not reported empty.
...
async fn main() {
    ...
    let auth_key = match std::env::var("TUNNEL_AUTH_KEY") {
        Ok(v) if !v.trim().is_empty() => v,
        _ => {
            if std::env::var("MHRV_AUTH_KEY")
                .map(|v| !v.trim().is_empty())
                .unwrap_or(false)
            {
                tracing::error!(
                    "MHRV_AUTH_KEY is set, but tunnel-node reads TUNNEL_AUTH_KEY. \
                     Rename the environment variable and keep it identical to CodeFull.gs."
                );
            }
            tracing::error!("TUNNEL_AUTH_KEY is required (refusing to start without it)");
            std::process::exit(2);
        }
    };
    ...
    let app = Router::new()
        ...
        .route("/health/details", get(handle_health_details))
        .with_state(state);
...
mod tests {
    ...
    #[test]
    fn health_details_advertises_full_tunnel_capabilities() {
        let health = health_details();

        assert_eq!(health.status, "ok");
        assert_eq!(health.service, "mhrv-f tunnel-node");
        assert_eq!(health.protocol, "mhrv-full-tunnel");
        assert_eq!(health.auth, "TUNNEL_AUTH_KEY");
        assert!(!health.version.trim().is_empty());
        assert!(health.supports_batch);
        assert!(health.supports_udp);
        assert!(health.supports_udpgw);
    }
    ...
    async fn drain_now_caps_at_tcp_drain_max_bytes() {
        ...
        inner
            ...
            .lock()
            .await
            .resize(TCP_DRAIN_MAX_BYTES + 4096, 0xab);

        let (first, eof) = drain_now(&inner).await;
        assert_eq!(first.len(), TCP_DRAIN_MAX_BYTES);
        assert!(!eof, "buffer tail must drain before eof can be reported");
    ...
    async fn drain_now_holds_eof_until_buffer_drained() {
        ...
        assert!(
            !head_eof,
            "premature eof would tear down a session with buffered data"
        );

        let (tail, tail_eof) = drain_now(&inner).await;
        assert_eq!(tail.len(), 100);
        assert!(tail_eof, "eof is reported after the tail drains");
    }
    ...
    // must engage LONGPOLL_DEADLINE, an active batch must stay bounded by
    // ACTIVE_DRAIN_DEADLINE + STRAGGLER_SETTLE_MAX, and `Some("")` must NOT
