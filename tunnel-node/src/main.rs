...
/// Structured error when the tunnel node receives an op it does not
/// implement. Kept in sync with `CodeFull.gs` and `domain_fronter::TunnelResponse`.
...
// ---------------------------------------------------------------------------
// Protocol types — single-op request
// ---------------------------------------------------------------------------
...
impl TunnelResponse {
    ...
    fn unsupported_op(name: &str) -> Self {
        Self {
            sid: None,
            d: None,
            eof: None,
            e: Some(format!("unknown op: {}", name)),
            code: Some(CODE_UNSUPPORTED_OP.to_string()),
        }
    }
}
...
async fn handle_batch(State(state): State<AppState>, body: Bytes) -> impl IntoResponse {
    ...
    // Phase 1: `connect` / `connect_data` each open a new upstream TCP (up
    // to the connect timeout). Run them concurrently so one slow target
    // does not head-of-line-block the whole batch.
...
/// Open a session and send the first payload in one step (used by the batch
/// path and the `connect_data` op).
async fn handle_connect_data_phase1(
    ...
    let session = match create_session(&host, port).await {
        Ok(s) => s,
        Err(e) => return Err(TunnelResponse::error(format!("connect failed: {}", e))),
    };
...
mod tests {
    ...
    #[test]
    fn unsupported_op_response_has_structured_code() {
        let resp = TunnelResponse::unsupported_op("nope");
        assert_eq!(resp.code.as_deref(), Some(CODE_UNSUPPORTED_OP));
        assert!(resp.e.as_deref().unwrap().contains("unknown op"));
    }

    #[test]
    fn validate_host_port_rejects_empty_and_zero() {
